# Documentação dos Diagramas de Atividades

## 1. Introdução

Este documento apresenta a consolidação dos artefatos de análise e projeto voltados ao domínio do software para Saúde e Segurança do Trabalho. Nas seções seguintes, são apresentados os seguintes trabalhos atualizados e revisados:

- Diagrama de Classes;
- Documentação dos Casos de Uso;
- Diagramas de Atividades.

## Justificativa de Escopo do Diagrama de Atividades: UC05 – Gerenciar Acidentes

Diante das diretrizes estabelecidas de não focar a modelagem em fluxos secundários de CRUD (Criação, Leitura, Atualização e Exclusão) e concentrar o detalhamento nos processos de real valor de negócio, o grupo optou por delimitar o escopo do Diagrama de Atividades do UC05 – Gerenciar Acidentes estritamente ao fluxo de Cadastro/Registro de Acidentes. Na correção da documentação dos casos de uso, o professor afirmou que o caso de uso supracitado acima, apesar de ser considerado secundário, é importante para a compreensão da lógica do sistema.

A fundamentação técnica para a exclusão das atividades de Consulta, Edição e Exclusão neste diagrama baseia-se nos seguintes pontos:

- Ao contrário de cadastros totalmente administrativos, o registro de um acidente de trabalho não é uma operação trivial. Neste caso, ele atua como o evento inicial de um fluxo complexo de conformidade legal. O ato de registrar “amarra” logicamente as classes Funcionario e Acidente conforme modelado no Diagrama de Classes.
- A modelagem do cadastro permite demonstrar o fluxo de controle requerido pelo professor nas últimas aulas: a verificação de permissões do perfil do GestorDeSeguranca, a validação do estado do Funcionario e a ramificação para fluxos de objetos (instanciação e persistência do objeto Acidente).
...
1. Acessar filtros
2. Selecionar um critério (ex: vencidos, próximos da manutenção)
3. Atualizar a lista exibida
...
### Cenário Execeção I -Falha de conexão com o banco de dados

**Ações do Ator / Ações do Sistema**

1. Tentar conexão com banco
2. Detectar falha de conexão
3. Exibir erro de indisponibilidade
4. Registrar erro no sistema
...
7. Dados devem estar atualizados
...
1. Acessar o módulo de áreas de risco e clica em “Realizar Simulação”
...
5. Validar a infraestrutura de saídas do setor.
6. Bloqueia virtualmente os sensores e saídas de emergência localizadas na zona de impacto
...
8. Atualiza as saídas na zona de impacto e volta para o passo 6 do cenário principal.
...
5. Verifica automaticamente a validade do Certificado de Aprovação (CA) junto à base de dados regulatória.
...
10. Gera uma requisição de compra automática para os itens cujo estoque mínimo foi atingido.
...
8. Estende a data de descarte calculada pelo sistema por um período determinado (respeitando o limite do CA).
...
9. Atualiza o valor total da requisição de compra.
...
2. Gera um alerta impeditivo no painel do Gestor e no aplicativo do Funcionário, exigindo a substituição imediata antes do início da jornada.
