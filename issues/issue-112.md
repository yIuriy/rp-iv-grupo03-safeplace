# Issue #112: [56] [MVP][Tarefas] Reconciliar NivelPerigo e classificar tarefas com autoria

- **Status:** Open
- **URL:** https://github.com/yIuriy/rp-iv-grupo03-safeplace/issues/112
- **Author:** @sidneicjr28 (Sidnei Jr)
- **Assignees:** None
- **Labels:** None
- **Milestone:** None
- **Created at:** 2026-09-11T01:21:33Z
- **Updated at:** 2026-09-15T01:02:47Z
...
**Requisitos e histórias:** RF06; UC11; US06.

**Referência do modelo:** Tarefa.descricao, status:NivelPerigo; associações com Setor/AreaRisco.

## O que entregar

Resolver vocabulário de UC03 versus UC11; mapear relacionamentos e completar classificação/reclassificação pelo Gestor com data, hora e responsável.

## Critérios de aceite

- [ ] Documentar se há vocabulário único ou correspondência explícita; não afirmar que PNG enum vazio define literais.
- [ ] Classificação e consulta usam contrato aprovado; alteração preserva histórico com responsável técnico.
- [ ] Tarefa sem classificação é identificável e sua alocação é impedida pela integração #117/#126.
- [ ] Supervisor consulta; Gestor define/reavalia conforme UC11.
- [ ] Não exigir vínculo tarefa–EPI no MVP nem inventar título, isAltoRisco ou regra normativa automática.
...
Vocabulário e representação das exigências normativas precisam de decisão registrada. Relações espaciais dependem do contrato #111.

## Motivo da correção

Escala harmonizada era afirmada sem decisão; campos e tipos de treinamento foram inventados na issue antiga.
...
O **RF06** e o **UC11** tratam da classificação do nível de periculosidade das tarefas a serem realizadas no ambiente de trabalho:
- Cadastrar tarefas operacionais com sua respectiva descrição e área de execução.
- Classificar a periculosidade da tarefa utilizando a escala harmonizada do sistema (`BAIXO`, `MEDIO`, `ALTO`, `CRITICO`).
- Estabelecer os requisitos de segurança e qualificações mínimas para a execução da tarefa.
...
## 3. O Que Fazer
1. Criar a entidade de domínio `Tarefa` em `domain.tarefa`:
   - Atributos: `id`, `titulo`, `descricao`, `areaRiscoId` (opcional), `nivelPericulosidade` (`BAIXO`, `MEDIO`, `ALTO`, `CRITICO`), `exigeTreinamentoObrigatorio`, `tipoTreinamentoExigido` (ex.: NR-10, NR-35).
2. Implementar métodos de negócio no domínio:
   - Identificar se a tarefa é de alto risco (`isAltoRisco()`).
   - Validar preenchimento dos requisitos de segurança para tarefas críticas.
3. Declarar as portas formais:
   - `port.in.ClassificarTarefaUseCase`.
   - `port.out.TarefaRepositoryPort`.

## 4. Como Fazer (Arquitetura Hexagonal)
- Isolar as regras de negócio de tarefas no domínio puro.
- Testes unitários cobrindo classificações e regras de treinamento exigido.
...
- [ ] Entidade `Tarefa` com escala de perigo consistente com as áreas de risco.
- [ ] Portas declaradas em `application.port`.
- [ ] Testes unitários validando regras de tarefas críticas.
